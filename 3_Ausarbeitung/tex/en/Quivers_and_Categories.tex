...
\begin{remark}{(Endomorphism monoid; explicitly cyclic monoid)}
Let $\mathcal{C}$ be a category and $M \in \mathcal{C}_{0}$ an object. Then the \ul{endomorphism set} $\mathrm{End}_{\mathcal{C}}(M)$
together with the composition of morphisms as operation, and $1_{M}$ as identity element, is a monoid.
