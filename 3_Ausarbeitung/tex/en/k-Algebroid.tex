...
\[
\mathbf{CatReps_{\mathcal{C}}} = \textup{Hom}(\Bbbk\mathbf{-Algebroid_{\mathcal{C}}}, \Bbbk\mathbf{-Mat})
\]
...
\begin{example}{($\Bbbk\mathbf{-mat}$ is an Ab-category)}
For two natural numbers $m,n \in \mathbb{N} = \mathrm{Obj}\,\Bbbk\mathbf{-mat}$, the set of morphisms is
$\Bbbk^{m\times n}$, the set of $m \times n$-matrices. This is an abelian group:
\begin{itemize}
\item The identity (zero) element is the $m \times n$ zero matrix $\mathbf{0}$.
