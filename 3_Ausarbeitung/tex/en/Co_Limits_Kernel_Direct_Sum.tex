...
doctrines for our categories with such limits. The doctrine we are interested in is that of an abelian category with
...
leave projective and injective objects for later.

\subsubsection{Ab-categories}
We are starting simple with Ab-categories. All we need is a zero object.
...
\[
\begin{tikzcd}
K \arrow[rrr, "\mathrm{KernelEmbedding}(f)", hook]                                     &  &  & A \arrow[r, "f"] & B \\
T \arrow[rrru, "\tau"', bend right] \arrow[u, "{\mathrm{KernelLift}(f,\tau)}", dashed] &  &  &                       &  
\end{tikzcd}
\]
\end{definition}
...
\[
\begin{tikzcd}
A \arrow[r, "f"] & B \arrow[rrr, "\mathrm{CokernelProjection}(f)", two heads] \arrow[rrrd, "\tau"', bend right] &  &  & C \arrow[d, "{\mathrm{CokernelColift}(f,\tau)}", dashed] \\
                      &                                                                                                   &  &  & T                                                            
\end{tikzcd}
\]
\end{definition}
...
In an additive category $\mathcal{C}$ we define the \ul{image} of a morphism $f : A \rightarrow B \in \mathcal{C}_{1}$ as the 
