...
\end{tikzcd}
\]
is a pushout square.\endnote{Cited from \cite{[Annoying Precision]}.}
\end{enumerate}
\end{lemma}
...
\item The dual statement is that $\mathrm{Hom}(-,P)$ is left exact.
\end{enumerate}
\end{proof}
...
\texttt{UniversalMorphismIntoDirectSum}, \texttt{IdentityMorphism},\\
\texttt{UniversalMorphismFromDirectSum} and \texttt{PreCompose}. See
...
by the empty list \texttt{[ ]}? That's why the implementation in \homalgProject uses a special function
...
\begin{Verbatim}[commandchars=!@|,fontsize=\small,frame=single,label=Example]
  ...
  !gapprompt@gap>| !gapinput@Display( PreCompose( iota1, pi2 ) );|
   ...
   A morphism in Category of matrices over GF(3)
  !gapprompt@gap>| !gapinput@IsEqualForMorphisms( PreCompose( iota1, pi2 ),|
  !gapprompt@>| !gapinput@     ZeroMorphism( V3, V5 ) );|
  true
  ...
  !gapprompt@gap>| !gapinput@Display( PreCompose( iota2, pi2 ) );|
   ...
   A morphism in Category of matrices over GF(3)
  !gapprompt@gap>| !gapinput@IsEqualForMorphisms( PreCompose( iota2, pi2 ),|
  !gapprompt@>| !gapinput@     IdentityMorphism( V5 ) );|
  true
...
\[
\mathtt{AdditionForMorphisms}
\]
can be derived from
\begin{align*}
\mathtt{UniversalMorphismIntoDirectSum}, \\
\mathtt{UniversalMorphismFromDirectSum},\\
\mathtt{IdentityMorphism}\,\text{ and} \\
\mathtt{PreCompose}.
\end{align*}
As an example we add two $\mathrm{GF}_{3}$-matrices from $\mathtt{V2}$ to $\mathtt{V3}$ in three
different ways.
...
\begin{Verbatim}[commandchars=!@|,fontsize=\small,frame=single,label=Example]
  ...
  <An identity morphism in Category of matrices over GF(3)>
  !gapprompt@gap>| !gapinput@result2 := PreCompose( |
  !gapprompt@>| !gapinput@   UniversalMorphismIntoDirectSum( [ one2, one2 ] ),|
  ...
  <An identity morphism in Category of matrices over GF(3)>
  !gapprompt@gap>| !gapinput@result3 := PreCompose( |
  !gapprompt@>| !gapinput@  UniversalMorphismIntoDirectSum( [ mor1, mor2 ] ),|
