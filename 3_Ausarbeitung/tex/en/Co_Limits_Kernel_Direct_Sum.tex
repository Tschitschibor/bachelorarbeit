...
A pre-Abelian category is Abelian if the natural morphism from $Coim$ to $Im$ is an isomorphism.
Therefore in the doctrine of abelian categories we will not need to distinguish between coimages and images and
the above diagrams simplify into an epi-mono factorization diagram.
...
\begin{itemize}
\item $\mathtt{ DirectSum ( D ) := VectorSpaceObject( Sum( List( D, V \mapsto Dimension( V ) ) ), k )}$
