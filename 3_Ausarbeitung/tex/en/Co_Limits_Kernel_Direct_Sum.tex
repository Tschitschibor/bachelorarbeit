...
\end{align*}
together with the algorithms from $\mathtt{IsAbCategory}$, then we can calculate all the algorithms in the doctrine
$\mathtt{IsAdditiveCategory}$:

The direct sum of the objects $\mathtt{D := [ V1, V2, \dots, VN ]}$ in $\kmat$ is defined as
\begin{itemize}
\item $\mathtt{ DirectSum ( D ) := VectorSpaceObject( Sum( List( D, V \mapsto Dimension( V ) ) ), GF3 )}$
\item $\mathtt{ ProjectionInFactorOfDirectSum( D, i ) := StackMatrix( [ }$\\
$\quad\mathtt{ List( D[1 .. (i-1)], V \mapsto ZeroMorphism( V, D[i] ) ), }$\\
$\quad\mathtt{ IdentityMorphism( D[i] ),}$\\
$\quad\mathtt{ List( D[(i+1) .. N], V \mapsto ZeroMorphism( V, D[i] ) ) ] )}$
\item $\mathtt{ InjectionOfCofactorOfDirectSum( D, i ) := AugmentMatrix( [ }$\\
$\quad\mathtt{ List( D[1 .. (i-1)], V \mapsto ZeroMorphism( D[i], V ) ), }$\\
$\quad\mathtt{ IdentityMorphism( D[i] ), }$\\
$\quad\mathtt{  List( D[(i+1) .. N], V \mapsto ZeroMorphism( D[i], V ) ) ] )}$
\item $\mathtt{UniversalMorphismIntoDirectSum ( [ phi, psi ] ) := StackMatrix( [ phi, psi ] )}$
\item $\mathtt{UniversalMorphismFromDirectSum ( [ phi, psi ] ) := AugmentMatrix( [ phi, psi ] )}$
\end{itemize}
...
different direct sums involved, one of the $m_{i}$'s and one of the $n_{i}$'s.
...
Let $\varphi : m \rightarrow n \in \kmat_{1}$ be a matrix (with transposed matrix $\varphi^{T} : n \rightarrow m$).
We assume algorithms for 
\begin{itemize}
...
as the $\Bbbk$-dimension of the row space $\mathrm{dim_{Row}}( \varphi ) = \mathrm{dim_{Col}}( \varphi^{T} ) = r$,
\item The left nullspace of a matrix, $N( \varphi^{T} ) := \mathtt{LeftNullSpace( phi )}$, defined as the subspace
of $\Bbbk^{m\times 1}$ that is the solution to $\varphi^{T}\, y^{T} = 0 \Leftrightarrow y\, \varphi = 0$, $y : k \rightarrow m$
for some $k \in \kmat_{0}$.
\item The right nullspace of a matrix, $N( \varphi ) := \mathtt{RightNullSpace( phi )}$, defined as the subspace of $\Bbbk^{1\times n}$
that is the solution to $\varphi\,x = 0$, $x : n \rightarrow c$ for some $c \in \kmat_{0}$.
...
\item Given the row-echolon-form $\mathtt{Solve( y \cdot REF( LeftNullSpace( phi ) ) = 0 )}$
as the solution to the homogeneous equation $y\, \varphi = 0$, we get the solution to the inhomogeneous equation
$y\, \varphi = \tau$ as $\mathtt{Solve( y \cdot REF( LeftNullSpace( phi ) ) = tau )}$.
\item Given the column-echolon-form $\mathtt{Solve(CEF( RightNullSpace( phi ) ) \cdot x = 0 )}$
as the solution to the homogeneous equation $\varphi\, x= 0$, we get the solution to the inhomogeneous equation
$\varphi \,x= \tau$ as $\mathtt{Solve( CEF( RightNullSpace( phi ) ) \cdot x = tau )}$. 
\end{itemize}
\end{computation}
...
The defining property of an Abelian category, that ``every monomorphism is a kernel'' and ``every epimorphism is a cokernel'',
makes us put the monomorphism $\kappa$ in place of $\mathtt{REF( LeftNullSpace( phi ) )}$, and the epimorphism
$\varepsilon$ in place of $\mathtt{CEF( RightNullSpace( phi ) )}$ in \label{ex:kmat_pre-abelian}, giving us the two
algorithms needed for the doctrine $\mathtt{IsAbelianCategory}$.
...
Then with the algorithms from \label{comp:gauss-algorithms} we define
$\mathtt{ LiftAlongMonomorphism( kappa, tau ) := Solve( x \cdot kappa = tau )}$.
\item Let $\varepsilon : B \twoheadrightarrow A \in \kmat_{1}$ and $\tau : B \rightarrow T \in \kmat_{1}$ be as in
\ref{def:abelian_category}(2).
Then with the algorithms from \label{comp:gauss-algorithms} we define
$\mathtt{ ColiftAlongEpimorphism( epsilon, tau ) := Solve( epsilon \cdot x = tau )}$
\end{itemize}
