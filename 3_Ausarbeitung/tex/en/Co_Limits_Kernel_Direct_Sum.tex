...
\begin{itemize}
\item For matrices $(m \times 0)$ we write $\mathtt{ZeroMorphism(m, 0) := ZeroMatrix( m, 0 )}$,
\item For matrices $(0 \times n)$ we write $\mathtt{ZeroMorphism(0, n) := ZeroMatrix( 0, n )}$,
...
&\mathtt{Sum}( [  &&] ) = 0,
\end{alignat*}
for stacking two matrices with the same number of columns ($\widehat{=}$ same target) on top of each other
\begin{align*}
...
\end{align*}
and for aligning two matrices with the same number of rows ($\widehat{=}$ same source) next to each other
\begin{align*}
