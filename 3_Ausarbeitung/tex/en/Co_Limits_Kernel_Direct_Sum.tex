...
but the assumption that a category is skeletal could simplify computation in those categories tremendously.
...
\begin{itemize}
\item the object $n := \bigoplus_{i=1}^{N} n_{i} := \sum_{i=1}^{N} n_{i}$
...
We get the projection $\pi_{i}$ as a result of
\[
\mathtt{ProjectionInFactorOfDirectSum}( [ n_{1},\dots,n_{N} ], i ) := 
\]
...
Algorithms for the doctrine of 
