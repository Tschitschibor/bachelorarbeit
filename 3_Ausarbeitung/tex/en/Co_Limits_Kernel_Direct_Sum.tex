...
\begin{example}[Block-Diagonal matrices]\phantom{}\\
...
\item We can view each matrix $a_{i}$ as part of a family $a_{i} := \{ a_{j,i} : m_{j} \rightarrow n_{i} \}_{j\in I}$ of matrices with same
target $n_{i}$.
\[
a_{j,i} := \begin{cases}
...
\end{cases}
\]
And for those we can write them in the $m \times n_{i}$ matrix 
\[
u_{\text{out}}(a_{i}) = \begin{pmatrix}
a_{1,i} \\
\vdots \\
a_{i,i} \\
\vdots \\
...
\end{pmatrix}
\]
\item Next we can view the $u_{\text{out}}(a_{i})$ as family $\{u_{\text{out}}(a_{i}) : m \rightarrow n_{i}\}_{i\in I}$ of matrices
with the same source $m$ and thus we can write them in the $m \times n$ matrix
\[
u_{\text{in}}(u_{\text{out}}(a_{1}),\dots,u_{\text{out}}(a_{N})) = \begin{pmatrix}
...
\end{pmatrix}
\]
which we call $\mathrm{Diag}(a_{1},\dots,a_{N})$.
